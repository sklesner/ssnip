...
async function requestSample(env, lesson, emoji) {
    let stuff = await env.ASSETS.fetch('https://joe.ackop.com/assets/story/sample.txt');
...
async function makeStory(env, lesson, emoji) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        answer = await requestSample(env, lesson, emoji)
