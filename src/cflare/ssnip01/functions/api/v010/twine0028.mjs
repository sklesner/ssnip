...
async function makeStory(env, lesson, emoji) {
  ...
  for (let i = 0; i < 4; i++) {
      ...
      storySpec = // STORY LLM CONFIG
        // await STATIC.sendStoryRequest(env, lesson, emoji);
...
async function genPictureSpec(env, storySpec) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    imageDesc = // PICTURE LLM CONFIG
      // await STATIC.sendPicturesRequest(env, storySpec);
...
export async function onRequest(context) {
  ...
  // TODO consider using redirect reply with URL emoji in url param
  return new Response(
