...
async function makeStory(env, lesson, emoji) {
  ...
  for (let i = 0; i < 4; i++) {
      ...
      // storySpec = await STATIC.sendStoryRequest(env, lesson, emoji);
      storySpec = await GPT.sendStoryRequest(env, lesson, emoji);
      // storySpec = await GROQ.sendStoryRequest(env, lesson, emoji);
...
async function loadTextFileAsync(env,filePath) {
  // try {
    ...
    var stuff = await env.ASSETS.fetch('https://joe.ackop.com/assets/gen/'+ filePath);
...
async function makeTwine(env,storyId,storyName,storySpec,emojiDelimited){
  var [headerTemplate, sceneTemplate, sceneUrlTemplate, footerTemplate] = await Promise.all([
    loadTextFileAsync(env,"twine_header.txt"),
    loadTextFileAsync(env,"twine_scene.txt"),
    loadTextFileAsync(env,"twine_scene_url.txt"),
    loadTextFileAsync(env,"twine_footer.txt")
  ]);
...
async function makeGeneratingHtml(env, emojiTxt) {
...
async function genPictureSpec(env, storySpec) {
  let imageDesc;

  for (let i = 0; i < 4; i++) {
    if (i > 0) console.log('pictures try: ' + (i - 1));

    // imageDesc = await STATIC.sendPicturesRequest(env, storySpec);
    imageDesc = await GPT.sendPicturesRequest(env, storySpec);
    // imageDesc = await GROQ.sendPicturesRequest(env, storySpec);
    // imageDesc = await CLAUDE.sendPicturesRequest(env, storySpec);
...
async function genQuestionSpec(env, storySpec) {
  let q1, q2;
  ...
  while ((!q1Valid || !q2Valid) && attempts < maxAttempts) {
    ...
    if (!q1Valid) {
      try {
        ...
        q1 = await STATIC.sendQuestion1Request(env, storySpec);
    ...
    if (!q2Valid) {
      try {
        ...
        q2 = await STATIC.sendQuestion2Request(env, storySpec);
...
async function makeQuizTwine(env, storyName, questionSpec) {
  var [headerTemplate, questionTemplate, responseSceneTemplate, responseUrlTemplate, footerTemplate] = await Promise.all([
    loadTextFileAsync(env,"twine_header.txt"),
    loadTextFileAsync(env,"twine_question.txt"),
    loadTextFileAsync(env,"twine_response_scene.txt"),
    loadTextFileAsync(env,"twine_response_url.txt"),
    loadTextFileAsync(env,"twine_footer.txt")
  ]);
...
function makeReviewScene(template, storyId, pageNum, imageUrl, imageWords, storyWords, musicUrl, musicWords, soundUrl, soundWords, emoji) {
  return template
    .replace(/<storyId>/g,storyId)
    .replace(/<pageNum>/g, pageNum)
    .replace(/<imageUrl>/g, imageUrl)
    .replace(/<imageWords>/g, imageWords)
    .replace(/<storyWords>/g, storyWords)
    .replace(/<musicUrl>/g, musicUrl)
    .replace(/<musicWords>/g, musicWords)
    .replace(/<soundUrl>/g, soundUrl)
    .replace(/<soundWords>/g, soundWords)
    .replace(/<emoji>/g, emoji)
    ;
}

function makeReviewHeader(template) {
  return template;
}

function makeReviewFooter(template) {
  return template;
}

async function makeReview(env) {
  var [reviewHeaderTemplate, reviewSceneTemplate, reviewFooterTemplate] = await Promise.all([
    loadTextFileAsync(env,"story_review/review_header.txt"),
    loadTextFileAsync(env,"story_review/review_scene.txt"),
    loadTextFileAsync(env,"story_review/review_footer.txt"),
  ]);
...
export async function onRequest(context) {
  ...
  const backgroundWork = async () => {
    const lesson = await getLesson(env)
    console.log("Final lesson:", lesson); // Debug log
    const storySpec = await makeStory(env,lesson,storyName);
    const storyId = await makeHash(storySpec)
    const storyTwine = await makeTwine(env,storyId,storyName,storySpec,emojiDelimited);
    ...
    const pictureSpec = await genPictureSpec(env, storySpec);
    await twineImageGen(env,pictureSpec,storyId);

    const questionSpec = await genQuestionSpec(env, storySpec);
    const quizTwine = await makeQuizTwine(env, storyName, questionSpec);
  ...
  return new Response(
    // storyTwine
    await makeGeneratingHtml(env,emojiDelimited)
    , {
    status: 200,
    headers: {
