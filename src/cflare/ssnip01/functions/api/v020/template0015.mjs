...
export function makeTwineScene(ctx, storyId, pid, pageName, prevPageName, nextPageName, sceneText, emoji) {
  const sceneText2 = 
    ...
    .replace(/(?<!\b(Mr|Mrs|Miss|Dr|Ms))([\!\.\?]['"]*) +(['"]*(?!Mr\.|Mrs\.|Miss\.|Ms\.|Dr\.)[A-Z])/g,'$2&lt;br&gt;&lt;br&gt;$3')
    .replace(/(?<!\b(Mr|Mrs|Miss|Dr|Ms))([\!\.\?]) +([A-Z])/g,'$2&lt;br&gt;&lt;br&gt;$3')
  ;

  const soundUrl = 'https://aipif-2023.s3.amazonaws.com/_asset/246cbccb.mp3';
  const musicUrl = 'https://aipif-2023.s3.amazonaws.com/_asset/aa1a443d.mp3';

  return ctx.txtFileMap.get("twine_scene")
    .replace(/<storyId>|{storyId}/g,storyId)
    .replace(/<pid>|{pid}/g, pid)
    .replace(/<pageName>|{pageName}/g, pageName)
    .replace(/<prevPageName>|{prevPageName}/g, prevPageName)
    .replace(/<nextPageName>|{nextPageName}/g, nextPageName)
    .replace(/<sceneText>|{sceneText}/g, sceneText2)
    .replace(/<emoji>|{emoji}/g, emoji)
    .replace(/<soundUrl>|{soundUrl}/g, soundUrl)
    .replace(/<musicUrl>|{musicUrl}/g, musicUrl)
    ;
}
...
export function makeTwineQuestion(ctx, storyId, pid, picId, emoji, pageName, questionText,
  ...
  .replace(/<storyId>|{storyId}/g, storyId)
  .replace(/<pid>|{pid}/g, pid)
  .replace(/<picId>|{picId}/g, picId)
  .replace(/<emoji>|{emoji}/g, emoji)
  .replace(/<pageName>|{pageName}/g, pageName)
  .replace(/<questionText>|{questionText}/g, questionText)
  
  .replace(/<b1Text>|{b1Text}/g, b1Text)
  .replace(/<b1Logic>|{b1Logic}/g, b1Logic)
  
  .replace(/<b2Text>|{b2Text}/g, b2Text)
  .replace(/<b2Logic>|{b2Logic}/g, b2Logic)

  .replace(/<b3Text>|{b3Text}/g, b3Text)
  .replace(/<b3Logic>|{b3Logic}/g, b3Logic)

  .replace(/<b4Text>|{b4Text}/g, b4Text)
  .replace(/<b4Logic>|{b4Logic}/g, b4Logic)
...
export function makeCorrectButtonLogic(ctx, nextPage) {
  ...
  .replace(/<nextPage>|{nextPage}/g, nextPage)
}
...
export function makeTwineResponseUrl(ctx, pid, pageName, messageText, bText, bAction){
  ...
  .replace(/<pid>|{pid}/g, pid)
  .replace(/<pageName>|{pageName}/g, pageName)
  .replace(/<messageText>|{messageText}/g, messageText)
  .replace(/<bText>|{bText}/g, bText)
  .replace(/<bAction>|{bAction}/g, bAction)
}
...
export function makeTwine(ctx,storyId,storyName,storySpec,emojiDelimited){
  ...
  return makeTwineHeader(ctx, storyName)
    ...
    + makeTwineResponseUrl(ctx, 14, "F", "Story complete!\nScore: $correctCount ✔️ and $incorrectCount ❌", "Another story?", "https://joe.ackop.com/")
