...
function toggleCategory(path, isChecked) {
    ...
    // If toggling a parent, ensure all its descendants match its state
    ...
    // If a node is unchecked, ensure its parents get updated accordingly
    if (!isChecked) {
        const parts = path.split('.');
        while (parts.length > 1) {
            parts.pop();
            const parentPath = parts.join('.');
            const hasAnyChildSelected = Object.keys(selectedCategories).some(key => key.startsWith(parentPath + '.') && selectedCategories[key]);
            selectedCategories[parentPath] = hasAnyChildSelected;
        }
    }
